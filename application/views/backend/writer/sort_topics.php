                    ...
                    <div class="row" id="parent-div" data-plugin="dragula" data-containers='["topic-list"]'>
                        <div class="col-md-12">
                            <div class="bg-dragula p-2 p-lg-4">
                                <h5 class="mt-0"><?php echo get_phrase('list_of_topics'); ?>
                                    <button type="button" class="btn btn-outline-primary btn-sm btn-rounded alignToTitle" id="topic-sort-btn" onclick="sort()" name="button"><?php echo get_phrase('update_sorting'); ?></button>
                                </h5>
                                <div id="topic-list" class="py-2">
                                    <?php foreach ($topics as $topic) : ?>
                                        <!-- Item -->
                                        <div class="card mb-0 mt-2 draggable-item" id="<?php echo $topic['id']; ?>">
                                            <div class="card-body">
                                                <div class="media">
                                                    <div class="media-body">
                                                        <h5 class="mb-1 mt-0"><?php echo $topic['topic']; ?></h5>
                                                    </div> <!-- end media-body -->
                                                </div> <!-- end media -->
                                            </div> <!-- end card-body -->
                                        </div> <!-- end col -->
...
<script type="text/javascript">
    function sort() {
        ...
        $.ajax({
            url: '<?php echo site_url('writer/ajax_sort_topic/'); ?>',
            type: 'POST',
            data: {
                itemJSON: itemJSON
            },
            success: function(response) {
                ...
                success_notify('<?php echo get_phrase('topics_have_been_sorted'); ?>');
                setTimeout(
                    function() {
                        location.reload();
                    }, 1000);
