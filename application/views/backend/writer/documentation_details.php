    ...
    <div class="card">
      <div class="card-body">
        <h4 class="page-title"> <i class="mdi mdi-apple-keyboard-command title_icon"></i> <?php echo get_phrase('documentation_for') . ' ' . $article_details['article']; ?>
          <a href="<?php echo site_url('writer/topics/' . $product_details['slug']); ?>" class="btn btn-outline-secondary btn-rounded alignToTitle"><?php echo get_phrase('back_to_topics'); ?></a>
      ...
      <div class="card-body">
        ...
        <div class="mb-3">
          <?php foreach ($articles as $key => $article) : ?>
            <a href="<?php echo site_url('writer/documentation_details/' . $article['slug'] . '-' . $article['id']); ?>" class="btn <?php if ($article['id'] == $article_details['id']) : ?> btn-primary <?php else : ?> btn-outline-primary <?php endif; ?> btn-rounded">
              <?php echo $article['article']; ?>
            </a>
          <?php endforeach; ?>
        </div>
        <!-- Articles list ends-->
        <form class="" action="<?php echo site_url('writer/update_documentation'); ?>" method="post" enctype="multipart/form-data">
