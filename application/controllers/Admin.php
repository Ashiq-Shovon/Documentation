...
class Admin extends CI_Controller
{
    ...
    /**
     * product FUNCTION HANDLES ALL THE TASKS FOR product
     *
     * @param string $action
     * @param string $product_id
     * @return void
     */
    public function products($action = '', $product_id = '')
    {
        if ($action == 'add') {
            $this->product_model->add_product();
            $this->session->set_flashdata('flash_message', get_phrase('product_has_been_added_successfully'));
            redirect(site_url('admin/products'), 'refresh');
        } elseif ($action == 'edit') {
            $this->product_model->edit_product($product_id);
            $this->session->set_flashdata('flash_message', get_phrase('product_has_been_updated_successfully'));
            redirect(site_url('admin/products'), 'refresh');
        } elseif ($action == 'delete') {
            $this->product_model->delete_product($product_id);
            $this->session->set_flashdata('flash_message', get_phrase('product_has_been_deleted_successfully'));
            redirect(site_url('admin/products'), 'refresh');
        }
    ...
    /**
     * PRODUCT FILE FUNCTION HANDLES ALL THE TASKS FOR PRODUCT FILE
     *
     * @param string $action
     * @param string $product_id
     * @return void
     */
    public function product_file($action = '', $product_file_id = '')
    {
        if ($action == 'add') {
            $product_details = $this->product_model->add_product_file();
            $this->session->set_flashdata('flash_message', get_phrase('product_file_has_been_added_successfully'));
        } elseif ($action == 'delete') {
            $product_details = $this->product_model->delete_product_file($product_file_id);
            $this->session->set_flashdata('flash_message', get_phrase('product_has_been_deleted_successfully'));
        }
    ...
    public function topic_action($action = '', $topic_id = '')
    {
        ...
        redirect(site_url('admin/topics/' . $product_details['slug']), 'refresh');
    }
    ...
    /**
     * THIS FUNCTION IS RESPONSIBLE FOR MANAGING THE product PACAKAGES
     *
     * @param string $product_slug
     * @return void
     */
    public function packages($product_slug = "", $action = "", $package_id = "")
    {
        if (empty($product_slug)) {
            $this->session->set_flashdata('error_message', get_phrase('you_have_choose_an_product_first'));
            redirect(site_url('admin/products'), 'refresh');
        ...
        if ($action == 'add') {
            $product_details = $this->product_model->get_product_by_slug($product_slug)->row_array();
            $this->pricing_package_model->add_price_package();
            $this->session->set_flashdata('flash_message', get_phrase('price_package_has_been_added_successfully'));
            redirect(site_url('admin/packages/' . $product_details['slug']), 'refresh');
        } elseif ($action == 'edit') {
            $product_details = $this->product_model->get_product_by_slug($product_slug)->row_array();
            $this->pricing_package_model->edit_price_package($package_id);
            $this->session->set_flashdata('flash_message', get_phrase('price_package_has_been_updated_successfully'));
            redirect(site_url('admin/packages/' . $product_details['slug']), 'refresh');
        } elseif ($action == 'delete') {
            $product_details = $this->product_model->get_product_by_slug($product_slug)->row_array();
            $this->pricing_package_model->delete_price_package($package_id);
            $this->session->set_flashdata('flash_message', get_phrase('price_package_has_been_deleted_successfully'));
            redirect(site_url('admin/packages/' . $product_details['slug']), 'refresh');
        }
        ...
        $page_data['page_title'] = get_phrase('manage_pricing_packages');
        $page_data['pricing_packages'] = $this->pricing_package_model->get_all_pricing_packages('slug', $product_slug);
    ...
    public function update_documentation()
    {
        ...
        redirect(site_url('admin/documentation_details/' . $response['slug'] . '-' . $response['id']), 'refresh');
    }
    ...
    public function blog_categories($action = '', $blog_category_id = '')
    {
        if ($action == 'add') {
            ...
            redirect(site_url('admin/blog_categories/'), 'refresh');
        } elseif ($action == 'edit') {
    ...
    public function blog_action($action = "", $blog_id = "")
    {
        if ($action == "delete") {
            ...
            redirect(site_url('admin/blog_form/' . $blog_id), 'refresh');
        }
    }

    public function campaign_update()
    {
        $data['value'] = $this->input->post('campaign', false);
        $this->db->where('key', 'campaign_bar');
        $this->db->update('settings', $data);

        $this->session->set_flashdata('flash_message', get_phrase('blog_has_been_stored'));
        redirect(site_url('admin/campaign_bar'), 'refresh');
    }
    ...
    /**
     * THIS FUNCTION RETURN USERS LIST
     *
     * @param string $action
     * @return void
     */
    public function users($action = "", $user_id = "")
    {
        if ($action == 'add') {
            $this->user_model->add_user();
            $this->session->set_flashdata('flash_message', get_phrase('user_has_been_added_successfully'));
            redirect(site_url('admin/users/'), 'refresh');
        } elseif ($action == 'edit') {
            $this->user_model->update_user_profile($user_id);
            $this->session->set_flashdata('flash_message', get_phrase('user_has_been_updated_successfully'));
            redirect(site_url('admin/users/'), 'refresh');
        } elseif ($action == 'delete') {
            $response = $this->user_model->delete_user($user_id);
            if ($response)
                $this->session->set_flashdata('flash_message', get_phrase('user_has_been_deleted_successfully'));
            else
        ...
        $page_data['page_title']        = get_phrase('user_list');
        $this->load->view('backend/index', $page_data);
    }
    ...
    /**
     * THIS FUNCTION IS RESPONSIBLE FOR SHOWING ALL THE SALES REPORT
     *
     * @return void
     */
    public function sales_report()
    {
        ...
        $page_data['sales_data']        = $this->payment_model->get_all();
        $this->load->view('backend/index', $page_data);
    }
    public function campaign_bar()
    {
        $page_data['page_name']         = 'campaign_bar';
        $page_data['page_title']        = get_phrase('campaign_bar');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function select_article_to_export($product_slug = "")
    {
        ...
        if ($product_details->num_rows() > 0) {
            ...
            $this->load->view('backend/admin/select_article_to_export.php', $page_data);
        } else {
            return false;
        }
    }
    ...
    public function export_documentation()
    {
        ...
        $html = $this->load->view('backend/admin/documentationPdfView.php', $page_data, true);
    ...
    {
        $this->ad_model->ad_activity_on_frontend();
        $this->session->set_flashdata('flash_message', get_phrase('ad_activity_has_been_changed_successfully'));
        redirect(site_url('admin/ad_network'), 'refresh');
    }
    function ad_create()
    {
        $this->load->view('backend/admin/ad_create');
    }

    function ad_edit($ad_id = "")
    {
        $page_data['ad']  = $this->ad_model->get_ads($ad_id)->row_array();
        $this->load->view('backend/admin/ad_edit', $page_data);
    }

    function ad_network($param1 = "", $param2 = "")
    {
        if ($param1 == "add") {
            $this->ad_model->ad_create();
            $this->session->set_flashdata('flash_message', get_phrase('ad_has_been_created_successfully'));
            redirect(site_url('admin/ad_network'), 'refresh');
        }

        if ($param1 == "update") {
            //$param2 = ad_id
            $this->ad_model->ad_update($param2);
            $this->session->set_flashdata('flash_message', get_phrase('ad_has_been_updated_successfully'));
            redirect(site_url('admin/ad_network'), 'refresh');
        }

        if ($param1 == "status") {
            //$param2 = ad_id
            $this->ad_model->ad_status($param2);
            $this->session->set_flashdata('flash_message', get_phrase('ad_status_has_been_updated_successfully'));
            redirect(site_url('admin/ad_network'), 'refresh');
        }

        if ($param1 == "delete") {
            //$param2 = ad_id
            $this->ad_model->ad_delete($param2);
            $this->session->set_flashdata('flash_message', get_phrase('ad_has_been_deleted_successfully'));
            redirect(site_url('admin/ad_network'), 'refresh');
        }
        $page_data['page_name']  = 'ad_network';
        $page_data['page_title'] = get_phrase('ad_network');

        if (isset($_GET['product_id']) || isset($_GET['dimension_id'])) {
            $page_data['dimension_id']  = $_GET['dimension_id'];
            $page_data['product_id']  = $_GET['product_id'];
            $page_data['ads']  = $this->ad_model->get_filtered_ads();
        } else {
            $page_data['ads']  = $this->ad_model->get_ads();
        }
        $this->load->view('backend/index', $page_data);
    }
    //End ad network
